...
<%
	    ...
	    try{
         ...
         PreparedStatement ps=con.prepareStatement("select *  from manager where uname='"+uname+"' and password='"+pword+"' and status='"+status1+"'");
		 ...
		 session.setAttribute("pm_id",n);
		 String p=rs.getString("uname");
		 session.setAttribute("pm_name",p);
		...
		// System.out.println("email id"+bb);
	     ...
	     response.sendRedirect("productionmanager_home.jsp");
		
		 }
		 else
		 {%>
		 <script>alert("Still admin not accept your request(or) incorrect username and password");
		 window.location="production_manager.jsp";
		 </script>
	<%
		 }
